...
static constexpr float FRONT_IR_TRIGGER_L_CM = 28.0f;  // front-LEFT  IR: obstacle ahead
...
static constexpr float SIDE_US_BLOCKED_CM  = 17.0f;  // turret US side-check: corridor blocked
static constexpr float REAR_IR_BLOCKED_CM  = SIDE_US_BLOCKED_CM-2.0;  // rear side IR: obstacle in strafe path
static constexpr float US_FWD_TRIGGER_CM   = FRONT_IR_TRIGGER_R_CM-7;  // forward US: obstacle straight ahead

// ─── Motion timing / odometry ─────────────────────────────────────────────────
static constexpr unsigned long SIDE_CHECK_SETTLE_MS = 200;
...
static constexpr unsigned long SETTLE_PAUSE_MS    = 150;  // motor-stop pause before confirming
...
static bool  strafePathBlocked(bool usGated, const __FlashStringHelper *tag);  // one tick of strafe obstacle detection
...
// ─── strafePathBlocked — one tick of strafe-path obstacle detection ───────────
// Shared by EVERY strafing state (AP_STRAFING, AP_STRAFE_EXTRA, AP_NOGAP_STRAFE)
// so the logic is provably identical everywhere. While strafing, the obstacle
// detectors are:
//   • side ultrasonic — turret aimed at the strafe side (the leading edge)
//   • rear IR on the strafe side — catches the trailing corner
// A hit on EITHER counts as an obstacle. The single-tick jump filter rejects a
// lone spurious ping but ALWAYS advances the baseline, so it can never latch and
// silently swallow a real wall. The block is debounced with corridorBlockCount;
// returns true only after CORRIDOR_BLOCK_TICKS consecutive blocked ticks.
//   usGated == false suppresses ONLY the US for the initial turret-settle window;
//   the rear IR stays active the entire time.
// Requires: turret already pointing at the strafe side; strafeRight set; the
// caller resets corridorBlockCount / prevUsSide / prevRearIR when a strafe begins.
static bool strafePathBlocked(bool usGated, const __FlashStringHelper *tag) {
    ...
    bool usBlocked   = usGated && usValid &&
                       (usSide > 0.5f && usSide < SIDE_US_BLOCKED_CM);
    bool rearBlocked = rearValid &&
                       (rearIR > 0.5f && rearIR < REAR_IR_BLOCKED_CM);

    // Always advance the baseline (any in-range reading) so the jump filter only
    // ever suppresses a single spurious tick — it must never freeze.
    if (usSide > 0.5f) prevUsSide = usSide;
    if (rearIR > 0.5f) prevRearIR = rearIR;

    static unsigned long lastLog = 0;
    if (millis() - lastLog > 200) {
        lastLog = millis();
        Serial.print(tag);
        Serial.print(F(" usSide=")); Serial.print(usSide, 1);
        Serial.print(F(" rearIR=")); Serial.print(rearIR, 1);
        Serial.print(F(" blk="));    Serial.print((usBlocked || rearBlocked) ? 1 : 0);
        Serial.print(F(" cnt="));    Serial.println(corridorBlockCount);
    }

    if (usBlocked || rearBlocked) {
        if (++corridorBlockCount >= CORRIDOR_BLOCK_TICKS) return true;
    } else {
        corridorBlockCount = 0;
    }
    return false;
...
static void doApproach() {
    float heading = sensors.getGyroHeading();
    int PTleft = analogRead(PT_LEFT_PIN);
    int PTmiddle = analogRead(PT_CENTRE_PIN);
    int PTright = analogRead(PT_RIGHT_PIN);
    ...
    switch (approachSub) {
        ...
        case AP_DRIVE: {
            ...
            //     motors.SetDriveStraightTarget(FORWARD, hotspot.angle, 0.0f, 0.0f);
            // }
            motors.DriveToLight(PTleft, PTmiddle, PTright);
            digitalWrite(9, LOW);
        ...
        case AP_BLOW_OUT: {
            ...
            Serial.println("Blowing fire out");
            digitalWrite(9, HIGH);
            if (ptBrightest() > 500){
                firesExtinguished++;
        ...
        case AP_STRAFING: {
            // US gated for the turret-settle window; rear IR always active.
            bool usSettled = (millis() - strafeStartMs >= STRAFE_US_SETTLE_MS);
            bool timedOut  = (millis() - strafeStartMs >= STRAFE_TIMEOUT_MS);

            if (strafePathBlocked(usSettled, F("[STRAFE]"))) {
                Serial.println(F("[STRAFE] Corridor blocked"));
        ...
        case AP_STRAFE_EXTRA: {
            // Turret already settled at the strafe side → US active immediately.
            if (strafePathBlocked(true, F("[EXTRA]"))) {
        ...
        case AP_NOGAP_REVERSE: {
            ...
            if (distReached || timedOut) {
                ...
                // Aim the turret US at the strafe side (LEFT) and reset the
                // corridor-block debounce + jump-filter baselines, so the long
                // strafe below can actually see obstacles beside it.
                pointTurret(false);          // LEFT = strafe side
                corridorBlockCount = 0;
                prevUsSide = -1.0f;
                prevRearIR = -1.0f;
                motors.strafeLeft();
        ...
        case AP_NOGAP_STRAFE: {
            // Same side-US + rear-IR corridor check as every other strafe.
            // Without it the robot strafed the full NOGAP_STRAFE_MS regardless of
            // anything beside it. Turret is pointed LEFT (set on entry), and
            // strafeRight==false so strafePathBlocked() reads the rear-LEFT IR.
            bool usSettled = (millis() - nogapStrafeMs >= STRAFE_US_SETTLE_MS);
            bool blocked   = strafePathBlocked(usSettled, F("[NOGAP_STRAFE]"));

            if (blocked || (millis() - nogapStrafeMs >= NOGAP_STRAFE_MS)) {
                motors.stop();
                Serial.println(blocked ? F("[NOGAP_STRAFE] Blocked → turret-seek")
                                       : F("[NOGAP_STRAFE] Done → turret-seek"));
                turret_motor.write(TURRET_FWD);
...
void DisplayAllSensors() {
    ...
    // Only print every 300ms to keep the Serial Monitor readable and responsive
    if (currentTime - lastPrintTime >= 200) {
        lastPrintTime = currentTime;
        turret_motor.write(TURRET_LEFT);
        
        // 2. Infrared Distance Sensors
        Serial.print(F("US: ")); Serial.print(sensors.pingNowCm(), 1);
        Serial.print(F("  FL: ")); Serial.print(sensors.readIRFrontLeft(), 1);
        Serial.print(F("  FR: ")); Serial.print(sensors.readIRFrontRight(), 1);
        Serial.print(F("  RL: ")); Serial.print(sensors.readIRRearLeft(), 1);
